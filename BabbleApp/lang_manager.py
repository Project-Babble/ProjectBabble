...
class LocaleStringManager:
    ...
    def _initialize(self, locale_directory, language):
        self._instance._languages = [lang for lang in os.listdir(locale_directory) if os.path.isdir(os.path.join(locale_directory, lang))]
    ...
    def _load_language(self, language):
        ...
        if self._instance._current_language not in self._instance._languages:
            raise ValueError(f"Language '{self._instance._current_language}' from settings is not supported")
    ...
    def get_string(self, pattern):
        ...
        if pattern not in self._instance._strings[self._instance._current_language]:
            raise KeyError(f"String pattern '{pattern}' not found for current language '{self._instance._current_language}'")
