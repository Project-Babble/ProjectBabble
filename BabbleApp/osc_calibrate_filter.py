    ...
    def cal_osc(self, array):
        ...
        if self.calibration_frame_counter == 0:
            if self.settings.calibration_mode == 'Full':
                ...
                print(f'[{lang._instance.get_string("log.info")}] Calibration completed.')
            ...
            if self.settings.calibration_mode == 'Neutral':
                ...
                num_outputs = values.shape[1]
                self.min_max_array = np.zeros((2, num_outputs))
                lower_threshold = np.clip([np.mean(values, axis = 0) + deadzone_value], 0, 1).tolist()[0]
        ...
        self.raw_array = array
        if self.settings.calib_array is not None and self.settings.use_calibration and self.settings.calibration_mode == 'Full':
            self.min_max_array = np.fromstring(self.settings.calib_array.replace('[', '').replace(']', ''), sep=',')
            self.min_max_array = self.min_max_array.reshape((2, 45))
        ...
        if self.settings.calib_array is not None and self.settings.use_calibration and self.settings.calibration_mode == 'Neutral':
            self.min_max_array = np.fromstring(self.settings.calib_array.replace('[', '').replace(']', ''), sep=',')
            self.min_max_array = self.min_max_array.reshape((2, 45))
