...
size_zoom = 300
# get the size of zoomed image from first argument
if len(sys.argv) > 1:
    size_zoom = int(sys.argv[1])
...
while True:
    ...
    centerx, centery = [int(frame.shape[0] / 2) - int(size_zoom / 2) , int(frame.shape[1] / 2) - int(size_zoom / 2)]
    frame = frame[centerx:centerx + size_zoom, centery:centery + size_zoom]
    ...
    #multiply by 100 for unity format, for neos - comment this line for 1x format
    ...
    #show the image
    gray = cv2.resize(gray, (size_zoom, size_zoom), interpolation = cv2.INTER_AREA)
